...
public:
    ...
    */
     ...
     *   odom里程计数据：
     *   1) 遍历当前激光帧起止时刻之间的odom里程计数据，初始时刻对应odom里程计设为当前帧的初始位姿
     *   2) 用起始、终止时刻对应odom里程计，计算相对位姿变换，保存平移增量
     * 3、当前帧激光点云运动畸变校正
     *   1) 检查激光点距离、扫描线是否合规
     *   2) 激光运动畸变校正，保存激光点
    ...
    */
    void imuDeskewInfo()
    {
        ...
        {
            //hwh 保留距离Lidar时间10ms以内的数据，太早的就不要了 (这里算一个工程经验吧)
    ...
    */
    void odomDeskewInfo()
    {
        ...
        for (int i = 0; i < (int)odomQueue.size(); ++i)
        {
            ...
            //hwh 因为已经剔除了该Lidar前10ms的数据，这里在寻找一个最接近Lidar的Odom作为起始Odom
        ...
        for (int i = 0; i < (int)odomQueue.size(); ++i)
        {
            ...
            //hwh 寻找和结尾最近的一帧odom数据 (Odom频率应该是高频，不然不准，或者得用线性拟合)
        ...
        // 起止时刻odom里程计的相对变换
        //hwh 起始时刻为Tw1  终止时刻为Tw2,所以T12 = Tw1.inv() * Tw2
        ...
        *rotXCur = 0; *rotYCur = 0; *rotZCur = 0;
        ...
        if (pointTime > imuTime[imuPointerFront] || imuPointerFront == 0)
        {
            ...
            *rotZCur = imuRotZ[imuPointerFront];
            ...
            // 前后时刻插值计算当前时刻的旋转增量
            //hwh 线性差值 https://zhuanlan.zhihu.com/p/34492145
    ...
    */
    ...
    /**
     * 激光运动畸变校正
     * 利用当前帧起止时刻之间的imu数据计算旋转增量，odom里程计数据计算平移增量，进而将每一时刻激光点位置变换到第一个激光点坐标系下，进行运动补偿
    */
    ...
    */
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            //hwh 计算当前点云属于[0~Horizon_SCAN]哪一列， Horizon_SCAN/2为中间位置，对应90°
