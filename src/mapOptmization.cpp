...
public:
    ...
    */
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        //hwh mappingProcessInterval为0.15s, map频率为6.667hz
    ...
    */
    void updateInitialGuess()
    {
        ...
        ////  hwh 这里主要就是利用imu和imu里程计提供位姿初值，其中上一帧位姿利用IMU数据中的位姿(orientation)并不是积分来的，当前帧的位姿由imu里程计或imu(orientation)获得
        //hwh 上面理解有误，cloudInfo.imuRollInit 是直接读取IMU的orientation读取来的 
        //hwh            cloudInfo.initialGuessRoll是根据预计分的结果转化为orientation得来的，不是直接读取的IMU的值
        //hwh   根据这两个位姿计算位姿变化量transIncre，再作用与上一帧的位姿transTobe得到预测位姿transformTobeMapped
        ...
        if (cloudInfo.odomAvailable == true)
        {
            ...
            if (lastImuPreTransAvailable == false)
            {
                ...
                // 前一帧的位姿
                //!hwh 这里其实有两个前一帧，lastImuPreTransformation和transformTobeMapped都表示前一帧
                //hwh      lastImuPreTransformation为预积分得来的位姿，这里利用相邻两帧的预积分计算deltaPose，transformTobeMapped为优化后的位姿
                //hwh      用优化的位姿 * deltaPose得到估计的初始位姿
                //hwh 此时的transformTobeMapped为前一帧的位姿
                Eigen::Affine3f transTobe = trans2Affine3f(transformTobeMapped);
                // 当前帧的位姿
                Eigen::Affine3f transFinal = transTobe * transIncre;
                // 更新当前帧位姿transformTobeMapped
                //hwh 此时的transformTobeMapped为当前帧的位姿
        ...
        //hwh imu里程计和imu同时存在时，作者优先采用imu里程计，我觉得imu里程计计算的是相对位姿，imu直接得到的orientation对于6轴可能长时间会漂移
        //hwh 这里只用了imu的位姿(由orientation得来)
